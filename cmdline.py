...
class HyQuitter:
    ...
    def __call__(self, code=None):
        # exit()
        # allow systemexit or not? you need to configure.
        try:
            sys.stdin.close() # quitting!
        except:
            pass
        # exit()
        # recursion will fuck up.
...
class HyREPL(code.InteractiveConsole):
    ...
    def run(self):
        ...
        with filtered_hy_exceptions(), extend_linecache(self.cmdline_cache), completion(
            ...
            try:
                self.interact( ## what could go wrong here?
                    "Hy {version} using "
...
def cmdline_handler(scriptname, argv): # run a single file?
    ...
    if argv:
        if argv[0] == "-":
            ...
            filename = argv[0]
            set_path(filename)
            # print('did you run file at:', filename)
            # this exception hook is applied.
            try:
                sys.argv = argv
                with filtered_hy_exceptions(): # so this is it.
                    # print(runhy.__file__)
                    # this file is misleading.
...
def hy_main():
    sys.path.insert(0, "")
    # print("are you using hy?")
    try:
        # you must be using this cmdline_handler.
...
def hyc_main(): # this is to compile hy.
